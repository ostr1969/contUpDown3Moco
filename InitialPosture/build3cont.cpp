...
//create markers
	...
	auto& coordSet = osimModel.updCoordinateSet();
       double q1=-99.698,q2=23.435,q3=-43.203 ;
       double pelrot=-38.174,pelx=0,pely=0.861;
...
//name,mxvel,qfacdir, act exp constant,dact exp constant,default act,filname, coor
        ...
        auto* a3=new DelpActuator("hp",20.,1,.011,.068,0.0,"src/delp5.txt",&coordSet.get("q3_rot"));
        auto* a_1=new DelpActuator("am",16.,-1,.011,.068,0.0,"src/delp2.txt",&coordSet.get("q1_rot"));
...
//      350N/0.08m=4375
	double pullymass=.001,resting_length=0.4,stiffness=4454.76*0.,dissipation=0.01;
...
//create contact between low point and ground
    ...
    cout<<"actsize:"<<osimModel.upd_ForceSet().updActuators().getSize()<<endl;   
        ...
        coordinates[0].setValue(si, pelrot*Pi/180, true);
        coordinates[1].setValue(si,pelx, true);
        coordinates[2].setValue(si,pely , true);
        coordinates[3].setValue(si,q1*Pi/180, true);
        coordinates[4].setValue(si,q2*Pi/180, true);
        coordinates[5].setValue(si,q3*Pi/180, true);
        for (int i=0;i<6;i++) cout<<coordinates[i].getName()<<endl;
...
//find initial optimal values for initial state(after setting angles)
	...
	comang=atan((comP[1]-tipP[1])/(comP[0]-tipP[0]))*180/Pi;
	pelrot=pelrot+90-comang;
        coordinates[0].setValue(si, pelrot*Pi/180, true);

	//get coord index and value and return the solution of torques at location cind
	...
	//gradient search on torqe solution to find zero 
	auto grad=[&](double init,int coordind,int udotind){
        ...
        //
           pely=grad(pely,2,2);
	   cout<<"got pely="<<pely<<":"<<coordinates[2].getValue(si)<<endl;
	   cout<<"computing again with this pely\n";
	   ...
	   cout<<"computing after changing a1,2,3\n";
	   compT(2,pely);
