...
Model buildmodel(){
	...
	double F1max=k1*140*Pi/180*0.05;
	double F2max=F1max-data.doubles[12].val/0.05;
	double k2=(F2max-data.doubles[12].val/0.05)/(140*Pi/180*0.05);
	
        sp1.setStiffness(k2);
	sp1.set_resting_length(L1-data.doubles[12].val/0.05/k2);
...
//    STOFileAdapter::write(ts, "results/colo_traj.sto");
   ...
   std::vector<std::string> contactSpheres_r;
    ...
    std::stringstream inittou_string;
    inittou_string<< std::fixed << std::setprecision(0) <<data.doubles[12].val;

    char Gfiln[80]="results/IGRF";
    char filn[80]="Analyzes/Itraj";
        strcat(Gfiln,sp1s.c_str());strcat(Gfiln,".");strcat(Gfiln,sp2s.c_str());strcat(Gfiln,".");
        strcat(Gfiln,sp3s.c_str());strcat(Gfiln,".");
	strcat(Gfiln,inittou_string.str().c_str());
	strcat(Gfiln,".sto");
    ...
    ts.updTableMetaData().setValueForKey("init_tou",to_string(data.doubles[12].val)) ;

        strcat(filn,sp1s.c_str());strcat(filn,".");strcat(filn,sp2s.c_str());strcat(filn,".");
        strcat(filn,sp3s.c_str());strcat(filn,".");
	strcat(filn,inittou_string.str().c_str());
	strcat(filn,".sto");
