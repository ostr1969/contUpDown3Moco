...
Model buildmodel(){
	...
	kUnitSpring=torqueAt90/.05/.05/Pi*2;// 1273.27 not 4454.76
...
//	const ControllerSet &controllerSet = osimModel.getControllerSet();
	...
	c.log("limits:",q1L,q1H,q2L,q2H,q3L,q3H);
        ...
        //auto* pmass = new OpenSim::Body("body", 3.0, SimTK::Vec3(0), SimTK::Inertia(0));
        //osimModel.addBody(pmass);
	//auto sphere = new Sphere(0.01);
	//sphere->setName("sphere");
	//sphere->setColor(SimTK::Red);
	//pmass->attachGeometry(sphere);
	auto kneepulley = osimModel.updBodySet().get("PulleyBody1");
	kneepulley.setMass(3.0);
    ...
    ActuatorPowerProbe* powerProbe = new ActuatorPowerProbe(Array<string>("hm",1),false, 1);
    powerProbe->setOperation("integrate");
    powerProbe->setInitialConditions(Vector(1, 0.0));
    osimModel.addProbe(powerProbe);
...
int main() {
    ...
    // Model (dynamics).
    // -----------------
      problem.setModelAsCopy(osimModel);
    // Bounds.
    // -------
...
parRun=1;
 ...
 //MocoTropterSolver& solver=study.initTropterSolver();
    ...
    TimeSeriesTable outputTable =
	    analyzeMocoTrajectory<double>(osimModel, solution,
            {"/forceset/knee_spring\\|stretch",
	     "/forceset/hip_spring\\|stretch",
	     "/forceset/ankle_spring\\|stretch",
	     "/kp\\|power"});
    STOFileAdapter_<double>::write(outputTable, "outputs.sto");
...
//    STOFileAdapter::write(ts, "results/colo_traj.sto");
   ...
   std::vector<std::string> contactSpheres_r;
    ...
    STOFileAdapter::write(externalForcesTableFlat, "results/colo_forces.sto");
	...
	STOFileAdapter::write(externalForcesTableFlat, Gfiln);
    ...
    TimeSeriesTable ts=solution.exportToStatesTable();
...
//timSeriesToBinFile(controlTable,"results/colo_controls.bin");
 ...
 cout<<solution.getObjectiveTermByIndex(0)<<"\t"<<endl;
	...
	<<"         jump:"<<jumphight<<","<<jumphight-0.9214<<endl;
    if (parRun)
