...
int main(int argc, char *argv[]){
   ...
   // std::vector<double> col2{1., 1.};
    Vector actv{2,0.0};
   // DataTable timAct;
   // timAct.appendColumn("time", col);
   // timAct.appendColumn("act", col2);
   cout<<"timv:"<<timv<<endl;

   // Vector timv=timAct.getDependentColumnAtIndex(0);
   // Vector actv=timAct.getDependentColumnAtIndex(1);
    auto* controller = new PrescribedController();
    controller->setName("prescribed_controller");

    for (int i = 0; i < actuNames.size(); ++i) {
        auto* funL=new PiecewiseLinearFunction(timv.nrow(), &timv[0], &actv[0]);
 	...
 	//OpenSim::Constant f(0.3);
        ...
        for (int j=0;j<timv.nrow();j++)
        fwddebugLog<<timv[j]<<","<<actv[j]<<endl;
    }
...
try {
	...
	double sysMass =  osimModel.getMultibodySystem().getMatterSubsystem()
        ...
        STOFileAdapter::write(statesTable, "results/fwd0_states.sto");
