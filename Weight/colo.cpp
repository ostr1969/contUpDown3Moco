...
//	const ControllerSet &controllerSet = osimModel.getControllerSet();
	...
	auto& bodies=osimModel.updBodySet();
	bodies[0].set_mass(data.doubles[12].val/2);
        bodies[1].set_mass(bodies[0].get_mass());
...
parRun=1;
 ...
 //MocoTropterSolver& solver=study.initTropterSolver();
	...
	return 0;}
    auto& bodies=osimModel.updBodySet();
    double topexomass=bodies[0].get_mass();
    double botexomass=bodies[1].get_mass();
   ...
   std::vector<std::string> contactSpheres_r;
    ...
    string exomass=to_string(data.doubles[12].val);
    std::stringstream exomass_string;
    exomass_string<< std::fixed << std::setprecision(1) <<data.doubles[12].val;

    char Gfiln[80]="results/WGRF";
    char filn[80]="Analyzes/Wtraj";
        strcat(Gfiln,sp1s.c_str());strcat(Gfiln,".");strcat(Gfiln,sp2s.c_str());strcat(Gfiln,".");
        strcat(Gfiln,sp3s.c_str());strcat(Gfiln,".");
	strcat(Gfiln,exomass_string.str().c_str());strcat(Gfiln,".sto");
    ...
    ts.updTableMetaData().setValueForKey("total exo weight",to_string(data.doubles[12].val)) ;
	...
	strcat(filn,exomass_string.str().c_str());strcat(filn,".sto");
...
//timSeriesToBinFile(controlTable,"results/colo_controls.bin");
    ...
    cout<<bodies[0].getName()<<","<<bodies[1].getName()<<endl;
    cout<<"exo mass="<<topexomass+botexomass<<" for both legs total."<<endl;
